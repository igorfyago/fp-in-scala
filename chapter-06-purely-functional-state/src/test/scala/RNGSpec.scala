...
class RNGSpec extends FunSuite with PropertyChecks {
  ...
  test("sequence") {
    forAll { (rng: RNG, rands: RNG.Rand[Int]) =>
  ...
  test("flatMap") {
    forAll { rng: RNG =>
      val expected = Answers.RNG.flatMap(rng)
      val result = RNG.flatMap(rng)
  ...
  test("mapWithFlatMap") {
    forAll { rng: RNG =>
      val expected = Answers.RNG.map(rng)
      val result = RNG.mapWithFlatMap(rng)
  ...
  test("map2WithFlatMap") {
    forAll { rng: RNG =>
      val expected = Answers.RNG.map2(rng)
      val result = RNG.map2WithFlatMap(rng)
